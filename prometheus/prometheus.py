...
class NodeCollector(BaseLnCollector):
    def collect(self):
        ...
        info_labels = {
            k.replace("-", "_"): v for k, v in info.items() if isinstance(v, str)
        }
        ...
        fees_msat = int(
            info.get("fees_collected_msat", info.get("msatoshi_fees_collected", None))
        )
        yield GaugeMetricFamily(
...
class FundsCollector(BaseLnCollector):
    def collect(self):
        ...
        print(funds["outputs"])
        output_funds = sum([o["amount_msat"] // 1000 for o in funds["outputs"]])
        channel_funds = sum([c["our_amount_msat"] // 1000 for c in funds["channels"]])
...
class ChannelsCollector(BaseLnCollector):
    def collect(self):
        ...
        for c in channels:
            ...
            labels = [
                c["peer_id"],
                c.get("short_channel_id", c.get("channel_id")),
                alias,
            ]
            balance_gauge.add_metric(labels, c["to_us_msat"] // 1000)
            spendable_gauge.add_metric(labels, c["spendable_msat"] // 1000)
            ...
            out_msatoshi_fulfilled_gauge.add_metric(
                labels, int(c["out_fulfilled_msat"])
            )
...
plugin.add_option("prometheus-listen", "127.0.0.1:9750", "Address and port to bind to")
