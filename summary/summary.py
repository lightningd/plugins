...
Charset = namedtuple(
    "Charset", ["double_left", "left", "bar", "mid", "right", "double_right", "empty"]
)
draw_boxch = Charset("╟", "├", "─", "┼", "┤", "╢", "║")
draw_ascii = Charset("#", "[", "-", "+", "]", "#", "|")
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                plugin.log(
                    "[PeerThread] Peerstate wrote to datastore. Sleeping now...",
                    "debug",
                )
                time.sleep(plugin.avail_interval)
    ...
    def run(self):
        while True:
            try:
                ...
                for _ in range(5):
                    r = requests.get(
                        "https://www.bitstamp.net/api/v2/ticker/btc{}".format(
                            plugin.currency.lower()
                        ),
...
def to_fiatstr(msat: Millisatoshi):
    return "{}{:.2f}".format(
        plugin.currency_prefix, int(msat) / 10**11 * plugin.fiat_per_btc
    )
...
def append_header(table, max_msat):
    ...
    draw = plugin.draw
    table.append(
        "%c%-13sOUT/OURS %c IN/THEIRS%12s%c SCID           FLAG  BASE   PPM AVAIL  ALIAS"
...
def summary(plugin, exclude="", sortkey=None, ascii=None):
    ...
    utxos = [
        int(f["amount_msat"]) for f in funds["outputs"] if f["status"] == "confirmed"
    ]
    ...
    if plugin.fiat_per_btc > 0:
        ...
        reply["fees_collected"] += " ({})".format(
            to_fiatstr(info["fees_collected_msat"])
        )
...
def load_datastore(plugin):
    ...
    if not check_datastore(persist):
        ...
        return new_datastore()
    plugin.log(
        f"Reopened datastore '{datastore_key}' with {persist['r']} "
        f"runs and {len(persist['p'])} entries",
        "debug",
    )
    return persist
...
def init(options, configuration, plugin):
    ...
    if hasattr(pyln.client, "__version__") and version.parse(
        pyln.client.__version__
    ) >= version.parse("0.0.7.1"):
        plugin.draw = draw_boxch
    ...
    if "always-use-proxy" in config and config["always-use-proxy"]:
        ...
        if ":" not in paddr:
            paddr += ":9050"
        proxies = {"https": "socks5h://" + paddr, "http": "socks5h://" + paddr}
    else:
        proxies = None
...
plugin.add_option(
    "summary-currency", "USD", "What currency should I look up on btcaverage?"
)
plugin.add_option("summary-currency-prefix", "USD $", "What prefix to use for currency")
plugin.add_option(
...
plugin.add_option(
    "summary-ascii", False, "If ascii mode should be enabled by default", "flag"
)
plugin.run()
